...
CREATE TABLE IF NOT EXISTS `db_4rest`.`Precio` (
  ...
  `seleccionador` INT NULL COMMENT '11,22,33,44... solo se selecciona 4 prod d los 8 disp donde no se repita un mismo valor',
  `created_at` TIMESTAMP NULL,
  `updated_at` TIMESTAMP NULL,
...
-- -----------------------------------------------------
-- Table `db_4rest`.`tipoins`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `db_4rest`.`tipoins` (
  `id` INT NOT NULL AUTO_INCREMENT,
  `nombre` VARCHAR(100) NULL,
  `descripcion` TEXT NULL,
  `created_at` TIMESTAMP NULL,
...
CREATE TABLE IF NOT EXISTS `db_4rest`.`Insumo` (
  ...
  `tipoins_id` INT NULL,
  PRIMARY KEY (`id`),
  INDEX `fk_Insumo_tipoins1_idx` (`tipoins_id` ASC),
  CONSTRAINT `fk_Insumo_tipoins1`
    FOREIGN KEY (`tipoins_id`)
    REFERENCES `db_4rest`.`tipoins` (`id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;
...
CREATE TABLE IF NOT EXISTS `db_4rest`.`PedidoCompra` (
  ...
  `importeFinal` DECIMAL(10,2) NULL,
  `usuario_id` INT NULL COMMENT 'creado por tal usuario',
...
CREATE TABLE IF NOT EXISTS `db_4rest`.`detProd` (
  ...
  `cantidad` DECIMAL(10,2) NULL,
  `seleccionador` INT NULL COMMENT 'para seleccionar el frutimix escogido en el desayuno x example',
  `created_at` TIMESTAMP NULL,
  `updated_at` TIMESTAMP NULL,
